...
export default function TradeLogTable({
  ...
  function renderCell(trade: Trade, key: TradeKey) {
    ...
    if (key === "before_picture" || key === "after_picture") {
      ...
      return <a href={url} target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:text-indigo-300"><ExternalLink size={12} /></a>;
    }
  ...
  function renderEditCell(key: TradeKey, data: Record<string, unknown>, setData: (d: Record<string, unknown>) => void) {
    const val = data[key];
    if (BOOL_FIELDS.has(key)) {
      return <input type="checkbox" checked={!!val} onChange={(e) => setData({ ...data, [key]: e.target.checked })} className="accent-indigo-500 w-3 h-3" />;
    }
    if (key === "direction") {
      return <select value={String(val || "Long")} onChange={(e) => setData({ ...data, [key]: e.target.value })} className="bg-slate-900 border border-slate-600 rounded px-1 py-0.5 text-[10px] text-white w-full"><option>Long</option><option>Short</option></select>;
    }
    if (key === "result") {
      return <select value={String(val || "Win")} onChange={(e) => setData({ ...data, [key]: e.target.value })} className="bg-slate-900 border border-slate-600 rounded px-1 py-0.5 text-[10px] text-white w-full"><option>Win</option><option>Loss</option><option>BE</option></select>;
    }
    if (NUM_FIELDS.has(key)) {
      return <input type="number" step="any" value={val === undefined || val === null ? "" : String(val)} onChange={(e) => setData({ ...data, [key]: parseFloat(e.target.value) || 0 })} className="bg-slate-900 border border-slate-600 rounded px-1 py-0.5 text-[10px] text-white w-full" />;
    }
    return <input type={key === "trade_date" ? "date" : "text"} value={String(val || "")} onChange={(e) => setData({ ...data, [key]: e.target.value })} className="bg-slate-900 border border-slate-600 rounded px-1 py-0.5 text-[10px] text-white w-full" />;
  }
  ...
  return (
    <div>
        ...
        <div className="flex items-center gap-2 text-[10px] text-slate-500">
          ...
          {!addingNew ? (
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { setAddingNew(false); setNewTrade(newEmptyTrade()); }} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-700 hover:bg-slate-600 text-white text-xs rounded-lg transition-colors"><X size={12} /> Cancel</button>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Filter bar */}
      {showFilters && (
        <div className="flex flex-wrap gap-3 mb-4 p-3 bg-slate-900/50 border border-slate-700 rounded-lg">
          <div>
            <label className="block text-[10px] text-slate-500 mb-1">Account</label>
            <select value={accountFilter} onChange={(e) => setAccountFilter(e.target.value)} className="bg-slate-800 border border-slate-700 rounded px-2 py-1.5 text-xs text-white">
          ...
          <div>
            ...
            <select value={sessionFilter} onChange={(e) => setSessionFilter(e.target.value)} className="bg-slate-800 border border-slate-700 rounded px-2 py-1.5 text-xs text-white">
              <option value="all">All sessions</option>
              {sessions.map((s) => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-[10px] text-slate-500 mb-1">Result</label>
            <select value={resultFilter} onChange={(e) => setResultFilter(e.target.value)} className="bg-slate-800 border border-slate-700 rounded px-2 py-1.5 text-xs text-white">
          ...
          <div>
            <label className="block text-[10px] text-slate-500 mb-1">Sort by</label>
            ...
            <tr className="border-b border-slate-700">
              {COLUMNS.map((c) => (
                <th
                  ...
                  className={`text-left py-2 px-1.5 text-slate-400 font-medium whitespace-nowrap cursor-pointer hover:text-slate-200 transition-colors ${c.width}`}
                >
            ...
            {filtered.map((t) => (
              <>
                  ...
                  <td className="py-1.5 px-1.5">
                    {editingId === t.id ? (
                      <div className="flex gap-1">
                        ...
                        <button onClick={cancelEdit} className="p-1 text-slate-400 hover:text-white" title="Cancel"><X size={12} /></button>
                      </div>
                    ) : (
                      <div className="flex gap-1">
                        <button onClick={() => setExpandedId(expandedId === t.id ? null : t.id)} className="p-1 text-slate-500 hover:text-indigo-400 transition-colors" title="Expand evaluation">
                          {expandedId === t.id ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
                        </button>
                        <button onClick={() => startEdit(t)} className="p-1 text-slate-500 hover:text-indigo-400 transition-colors" title="Edit"><Pencil size={12} /></button>
                        <button onClick={() => deleteTrade(t.id)} className="p-1 text-slate-500 hover:text-red-400 transition-colors" title="Delete"><Trash2 size={12} /></button>
                      </div>
                    )}
                  </td>
                </tr>
                ...
                {expandedId === t.id && (t.trade_evaluation || t.notes) && (
                  <tr key={`${t.id}-eval`} className="border-b border-slate-700/50">
                    <td colSpan={COLUMNS.length + 1} className="py-3 px-4 bg-slate-900/30">
                      {t.trade_evaluation && (
                        <div className="mb-3">
                          <div className="text-[10px] text-indigo-400 font-semibold mb-1 uppercase tracking-wide">Trade Evaluation</div>
                          <p className="text-xs text-slate-300 leading-relaxed whitespace-pre-wrap">{t.trade_evaluation}</p>
                        </div>
                      )}
                      {t.notes && t.notes !== t.trade_evaluation && (
                        <div>
                          <div className="text-[10px] text-slate-500 font-semibold mb-1 uppercase tracking-wide">Notes</div>
