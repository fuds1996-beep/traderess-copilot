    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          { label: "Avg During Session", value: psych.avgEmotionDuring, color: "text-brand" },
        ...
        <div className="glass rounded-2xl p-5 border border-brand-light/40">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {psych.emotionPnlCorrelation.map((e) => (
              <div key={e.emotion} className="p-3 bg-brand-light/80 rounded-lg text-center">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          {!showReflection && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-brand hover:bg-brand-dark text-white text-xs rounded-lg transition-colors"
            >
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-white/50 border border-brand-light/40 text-gray-600 text-xs rounded-lg"
              >
                <X size={12} /> Cancel
              </button>
            </div>
            ...
            return (
              <div key={weekStart}>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-between px-4 py-2.5 bg-white/50 border border-brand-light/40 rounded-lg hover:bg-brand-light/60 transition-colors mb-2"
                >
                  <div className="flex items-center gap-2">
                ...
                {/* Entries */}
                {!isCollapsed && (
                  <div className="space-y-2 ml-2 border-l-2 border-brand-light/40 pl-4">
                    {entries.map((j) => {
                      ...
                      return (
                        <div key={j.id} className="glass rounded-2xl border border-brand-light/40 overflow-hidden">
                          <div className="flex items-center justify-between p-3">
                            {/* Action icons at start */}
                            <div className="flex items-center gap-1 shrink-0 mr-2">
                              <button onClick={() => { setEditingId(j.id); setEditData({ ...j }); }} className="p-1 text-gray-400 hover:text-brand transition-colors" title="Edit">
                                <Pencil size={12} />
                              </button>
                            ...
                            <button
                              ...
                              className="flex-1 flex items-center justify-between text-left hover:bg-brand-light/30 rounded-lg px-1 -mx-1 transition-colors"
                            >
                              ...
                              <div className="flex items-center gap-2">
                                <div className="hidden sm:flex gap-1 text-[10px]">
                                  <span className="px-1.5 py-0.5 bg-brand-light/60 rounded text-gray-500">{j.emotion_before || "—"}</span>
                                  <span className="text-gray-300">→</span>
                                  <span className="px-1.5 py-0.5 bg-brand-light/60 rounded text-gray-500">{j.emotion_during || "—"}</span>
                                  <span className="text-gray-300">→</span>
                                  <span className="px-1.5 py-0.5 bg-brand-light/60 rounded text-gray-500">{j.emotion_after || "—"}</span>
                                </div>
                          ...
                          {isExpanded && (
                            <div className="px-4 pb-4 border-t border-brand-light/40 pt-3 space-y-3">
                              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-2 text-xs">
                                <div className="p-2 bg-brand-light/80 rounded">
                              ...
                              {j.journal_text && (
                                <div>
                                  <div className="text-[10px] text-brand font-semibold mb-1 uppercase tracking-wide">Daily Journal</div>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white/95 backdrop-blur-xl border border-brand-light/40 rounded-2xl shadow-2xl shadow-brand/10 w-full max-w-2xl max-h-[85vh] overflow-y-auto">
            <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-xl border-b border-brand-light/30 px-6 py-4 flex items-center justify-between rounded-t-2xl">
              ...
              <button onClick={cancelEdit} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-brand-light rounded-lg"><X size={18} /></button>
            </div>
            ...
            <div className="sticky bottom-0 bg-white/95 backdrop-blur-xl border-t border-brand-light/30 px-6 py-4 flex items-center justify-end gap-3 rounded-b-2xl">
              ...
              <button onClick={saveEdit} disabled={saving} className="flex items-center gap-2 px-5 py-2 bg-brand hover:bg-brand-dark disabled:opacity-50 text-white text-sm font-medium rounded-xl shadow-md shadow-brand/20">
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white/95 backdrop-blur-xl border border-brand-light/40 rounded-2xl shadow-2xl shadow-brand/10 w-full max-w-2xl max-h-[85vh] overflow-y-auto">
            <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-xl border-b border-brand-light/30 px-6 py-4 flex items-center justify-between rounded-t-2xl">
              ...
              <button onClick={() => { setAddingNew(false); setNewEntry(emptyJournal()); }} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-brand-light rounded-lg"><X size={18} /></button>
            </div>
            <div className="px-6 py-5">
            ...
            <div className="sticky bottom-0 bg-white/95 backdrop-blur-xl border-t border-brand-light/30 px-6 py-4 flex items-center justify-end gap-3 rounded-b-2xl">
              ...
              <button onClick={addJournal} disabled={saving} className="flex items-center gap-2 px-5 py-2 bg-brand hover:bg-brand-dark disabled:opacity-50 text-white text-sm font-medium rounded-xl shadow-md shadow-brand/20">
...
function JournalEditorFields({ data, setData }: { data: Partial<DailyJournal>; setData: (d: Partial<DailyJournal>) => void }) {
  ...
  const cls = "w-full bg-white/60 border border-brand-light/40 rounded-lg px-3 py-2 text-sm text-gray-900 focus:outline-none focus:border-brand";
...
function JournalEditor({
  ...
  return (
    <div className="glass rounded-2xl p-5 border border-brand-light/50 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={onCancel} className="flex items-center gap-1.5 px-3 py-1.5 bg-brand-light/60 hover:bg-slate-600 text-gray-900 text-xs rounded-lg transition-colors">
            <X size={12} /> Cancel
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div>
          ...
          <input type="date" value={data.journal_date || ""} onChange={(e) => update("journal_date", e.target.value)}
            className="w-full bg-white/60 border border-brand-light/40 rounded px-2 py-1.5 text-xs text-gray-900 focus:border-brand focus:outline-none" />
        </div>
        <div>
          <label className="block text-[10px] text-gray-400 mb-1">Day</label>
        ...
        <div>
          ...
          <input type="text" value={data.market_mood || ""} onChange={(e) => update("market_mood", e.target.value)} placeholder="Overall bearish"
            className="w-full bg-white/60 border border-brand-light/40 rounded px-2 py-1.5 text-xs text-gray-900 placeholder-slate-600 focus:border-brand focus:outline-none" />
        </div>
        <div>
          <label className="block text-[10px] text-gray-400 mb-1">Fundamentals</label>
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full bg-white/60 border border-brand-light/40 rounded px-3 py-2 text-xs text-gray-900 placeholder-slate-600 focus:border-brand focus:outline-none resize-y leading-relaxed"
        />
      </div>
    </div>
  );
