        ...
        <div className="space-y-4">
          <div className="glass rounded-2xl p-5 border border-pink-200/40">
            ...
            <div className="space-y-3">
              {tradingConfig.map((s) => (
                <div key={s.label} className="flex items-center justify-between py-2 border-b border-pink-200/30 last:border-0">
                  <span className="text-xs text-gray-500">{s.label}</span>
                  <span className="text-xs text-gray-900 font-medium bg-pink-100/60 px-3 py-1 rounded">{s.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="glass rounded-2xl p-5 border border-pink-200/40">
            ...
            <div className="space-y-2">
              {settings.data_connections.map((c) => (
                <div key={c.name} className="flex items-center justify-between p-3 bg-pink-50/80 rounded-lg">
                  <div>
                    <div className="text-xs text-gray-900 font-medium">{c.name}</div>
                    <div className="text-[10px] text-gray-400">{c.desc}</div>
                  </div>
                  <button className={`text-xs px-3 py-1 rounded-lg ${c.connected ? "bg-emerald-50 text-emerald-500 border border-emerald-200/50" : "bg-pink-100/60 text-gray-500 hover:bg-pink-100"}`}>
          ...
          <div className="glass rounded-2xl p-5 border border-pink-200/40">
            ...
            <div className="space-y-2">
              {settings.news_sources.map((s) => (
                <div key={s.name} className="flex items-center justify-between p-2 bg-pink-50/80 rounded">
                  <div>
                    <div className="text-xs text-gray-900">{s.name}</div>
                    <div className="text-[10px] text-gray-400">{s.url}</div>
                  </div>
                  <ToggleSwitch defaultOn={s.active} activeColor="bg-pink-500" />
                </div>
              ))}
            </div>
          </div>
